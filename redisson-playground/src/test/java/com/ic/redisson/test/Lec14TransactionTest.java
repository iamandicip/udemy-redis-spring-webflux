...
public class Lec14TransactionTest extends BaseTest {
    ...
    @AfterAll
    public void accuntBalanceStatus() {
    ...
    public void nonTransactionTest(){
        ...
        this.transfer(this.user1Balance, this.user2Balance, 50)
                .thenReturn(0)
                .map(i -> (5 / i)) // cause some error
                .doOnError(System.out::println)
                .subscribe(); // the transfer will still be successful
    ...
    private Mono<Void> transfer(RBucketReactive<Long> fromAccount, RBucketReactive<Long> toAccount, int amount) {
        return Flux.zip(fromAccount.get(), toAccount.get()) // [balance1, balance2]
